...
class BibTexVolumeTest extends SearchTestBase {
  ...
  static void init() {
    ...
    IntStream.range(0, NUMBER_OF_TEST_PUBLICATIONS)
        ...
        .forEach(
            i -> {
              var title = "BibTex volume test using tokens "
                  + i
                  + " "
                  + VOLUME_UUID
                  + " "
  ...
  @Description("A publication in BibTex format should have headers X-Total-Count and"
  ...
  void shouldReturnAllPublicationsInBibTexFormat() {
    ...
    softly.assertThat(response.header("X-Total-Count")).isNotEmpty();
    softly
        .assertThat(response.header("X-Total-Count"))
  ...
  @Description("A search that returns a number of hits less than the size parameter should not return a 'Link' header")
  ...
  @Description("A search that returns no hits should return an empty body, size=0 and no 'Link' header")
  void shouldReturnEmptyBodyWhenSearchReturnsNoHits() {
    ...
    softly.assertThat(response.header("X-Total-Count")).isEqualTo("0");
