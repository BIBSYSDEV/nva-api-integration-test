...
class BibTexTest extends SearchTestBase {
  ...
  private String createTestPublication(Category category, String title) {
    return switch (category) {
      case ACADEMIC_CHAPTER -> {
        var anthologyIdentifier = PUBLICATION_FACTORY.createAnthologyForChapter(
            UIB_CREATOR,
  ...
  private void waitForIndexing(String query) {
    with()
        ...
        .await()
        .atMost(30, SECONDS)
        .until(() -> !getResponseBody(query).isEmpty());
  }

  @ParameterizedTest
  @MethodSource("publicationsInBibTexFormatProvider")
  @DisplayName("Search with content type 'text/x-bibtex' produces BibTeX export for customer")
  @Description("Search returned with content type 'text/x-bibtex' is correct BibTex-format for customer")
  void shouldReturnPublicationsInBibTexFormatForCustomer(Category category, BibTexExpectation expectation) {
  ...
  @Description("A list of publications should contain a type that starts with '@' and two newlines between"
      + " publications")
  ...
  @Description("A publication with both onlineIssn and printIssn should only return onlineIssn in BibTex format")
  void shouldReturnOnlineIssnWhenBothOnlineIssnAndPrintIssnIsPresent() {

    final String ONLINE_ISSN = "1520-4898";
    var titleUuid = UUID.randomUUID().toString();
    ...
    assertThat(response.lines()).contains("  issn = {" + ONLINE_ISSN + "},");
  }

  private void createIssnPublication(String title) {
    final String ISSN_JOURNAL_URI = baseURI
        + "/publication-channels-v2/serial-publication/271CEF41-0052-48CA-BB31-6780C7BA1F44/" + CURRENT_YEAR;

    var response = PUBLICATION_FACTORY.createDraftPublication(UIB_CREATOR);
    String identifier = response.body().jsonPath().get("identifier");
    Map<String, Object> payload = response.body().jsonPath().getMap("");
    payload.remove(PublicationFields.CONTEXT_FIELD);
    var entityDescription = PUBLICATION_FACTORY.createEntityDescription(title, ACADEMIC_ARTICLE, List.of(UIB_CREATOR));

    ((Map<String, Object>) ((Map<String, Object>) entityDescription.get("reference")).get("publicationContext"))
        .put("id", ISSN_JOURNAL_URI);
  ...
  @Description("A publication with multiple authors should present a list of authors separated with 'and'")
  void shouldPresentMultipleAuthorsSeparatedWithAnd() {
    ...
    PUBLICATION_FACTORY.createPublishedPublication(UIB_CREATOR, title, ACADEMIC_ARTICLE,
        List.of(UIB_CREATOR, UIB_CONTRIBUTOR, UIB_PUBLISHING_CURATOR), UIB_PUBLISHING_CURATOR);

    waitForIndexing(titleUuid);

    var response = getResponseBody(titleUuid);
    String authorLine = response.lines().filter(line -> line.contains("author")).findFirst().toString().trim();
    ...
    softly.assertThat(authorLine).isEqualTo("author = {" + UIB_CREATOR.name() + " and " + UIB_CONTRIBUTOR.name()
        + " and " + UIB_PUBLISHING_CURATOR.name() + "},");
  }

  @Test
  @DisplayName("Keywords are joined with ','")
  @Description("A publication with multiple keywords should present a list of keywords separated with ','")
  void shouldPresentMultipleKeywordsSeparatedWithComma() {
    ...
    String keywordLine = searchResponse.lines().filter(line -> line.contains("keywords")).findFirst().toString().trim();
    ...
    softly.assertThat(keywordLine).isEqualTo("keywords = {key1,key2,key3},");
